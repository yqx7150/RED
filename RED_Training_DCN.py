...
from nnet.Model_Utility import MSELoss, normalize, batch_psnr, batch_ssim, batch_nrmse, load_config

from torch.utils.tensorboard import SummaryWriter

def train_dcn(params):
    ...
    pre_trained_diffusion = params["pre_trained_diffusion"]
    assert pre_trained_diffusion is not None, print("Please specify a pre-trained REN model and retry.") 

    diffusion = get_diffusion(max_timestep = max_timestep, path = pre_trained_diffusion,device=device).to(device)
    ...
    for epoch in range(epoch_num):
        ...
        for batch_idx, batch in progress_bar:
            ...
            x_T = low_dose_sino
            x_0 = norm_dose_sino
            real_residua = x_T - x_0

            with torch.no_grad():
                pred_residual = diffusion.nn_ren(x_T, ts / max_timestep)
                r = (torch.rand(batch_size, 1, 1, 1) * 10).to(device)
                pred_residual = r * pred_residual
                drift_x_t = diffusion.degrade(x_0,pred_residual,ts)
                x_t = diffusion.degrade(x_0,real_residua,ts)
                drift = x_t - drift_x_t
            ...
            writer.add_scalar(f'Train/Loss_2_Drift', loss.item(), global_step)
            
            pred_x_0 = (drift_x_t + pred_drift) - alpha_t * pred_residual

            writer.add_scalar(f'Train/Training PSNR', batch_psnr(pred_x_0, x_0).item(), global_step)
            writer.add_scalar(f'Train/Training SSIM', batch_ssim(pred_x_0, x_0).item(), global_step)
            writer.add_scalar(f'Train/Training NRMSE', batch_nrmse(pred_x_0, x_0).item(), global_step)
...
if __name__ == '__main__':
    ...
    config = load_config("./config_dcn.json")
    
    train_dcn(params = config)
